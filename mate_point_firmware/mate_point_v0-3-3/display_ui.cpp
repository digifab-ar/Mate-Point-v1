...
static void on_parar_clicked(lv_event_t *e)
{
    (void)e;
    if (parar_cb) {
        parar_cb();
...
void display_ui_init()
{
    ...
    lv_obj_add_event_cb(btn_parar, on_parar_clicked, LV_EVENT_CLICKED, nullptr);
    lv_obj_add_flag(btn_parar, LV_OBJ_FLAG_HIDDEN);

    lv_obj_t *parar_label = lv_label_create(btn_parar);
    lv_label_set_text(parar_label, "Parar");
    lv_obj_set_style_text_font(parar_label, &lv_font_montserrat_44, 0);
    lv_obj_set_style_text_color(parar_label, lv_color_white(), 0);
    lv_obj_center(parar_label);
