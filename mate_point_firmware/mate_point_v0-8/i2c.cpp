...
bool DEV_I2C_Register_Device(uint8_t addr, i2c_master_dev_handle_t *out_dev)
{
    if (!out_dev || handle.bus == nullptr) {
        return false;
    }
    ...
    return i2c_master_bus_add_device(handle.bus, &i2c_dev_conf, out_dev) == ESP_OK;
}
