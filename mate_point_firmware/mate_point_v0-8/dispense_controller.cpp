...
static uint32_t pause_timeout_ms = PAUSE_DECISION_TIMEOUT_MS;
...
static void update_termo_debug_ui(const Vl6180xSample *sample)
{
    char line1[72];
    char line2[72];

    snprintf(line1, sizeof(line1), "Sensor: %s", vl6180x_init_status_text());

    if (sample->status == VL6180X_STATUS_OK) {
        ...
        snprintf(line2, sizeof(line2), "Dist: %u | %s",
                 (unsigned)sample->distance_mm,
                 vl6180x_status_text(sample->status));
    }
...
static void refresh_temp_ui()
{
    if (phase == READY_START) {
        display_ui_set_dispense_temp_c(UI_TEMP_DISPLAY_MIN_C, true);
        return;
    }

    uint8_t temp_c = 0;
    const bool temp_ok = nobana_live_temp_c(&temp_c);
    if (temp_ok && (phase == DISPENSING || phase == PAUSED)
        && temp_c < UI_TEMP_DISPLAY_MIN_C) {
        temp_c = UI_TEMP_DISPLAY_MIN_C;
    }
...
static bool poll_termo_transition(uint32_t now, bool want_present)
{
    ...
    if (phase == WAIT_TERMO || phase == WAIT_TERMO_RESUME) {
        update_termo_debug_ui(&sample);
    }

    const bool present = sample.status == VL6180X_STATUS_OK && sample.termo_present;
...
static void start_pause_deadline(uint32_t now)
{
    pause_deadline_ms = now + pause_timeout_ms;
}

static void show_paused_ui(uint32_t now)
{
    (void)now;
...
static void enter_paused_from_dispensing(uint32_t now)
{
    accumulate_dispensed_ms(now);
    nobana_dispense_abort_pause();
    start_pause_deadline(now);
...
static void enter_wait_termo_resume(uint32_t now, bool from_dispensing)
{
    if (from_dispensing) {
        accumulate_dispensed_ms(now);
        nobana_dispense_abort_pause();
...
static void enter_paused_from_wait_termo_resume(uint32_t now)
{
    phase = PAUSED;
    continuar_pending = false;
    reset_termo_streak();
...
static void enter_finalize(uint32_t now)
{
    if (phase != DISPENSING && phase != PAUSED && phase != WAIT_TERMO_RESUME) {
        return;
    }
...
static void reset_to_listo()
{
    ...
    segment_start_ms = 0;
    pause_deadline_ms = 0;
    pause_timeout_ms = PAUSE_DECISION_TIMEOUT_MS;
...
static bool session_dispensing_mqtt()
{
    return phase == DISPENSING || phase == PAUSED || phase == WAIT_TERMO_RESUME
           || phase == TERMINADO || phase == LISTO_WAIT;
}

const char *dispense_mqtt_state()
{
    if (session_dispensing_mqtt()) {
        return "dispensing";
    }
    return "idle";
}
                         ...
                         uint32_t pause_timeout_in)
{
    ...
    pause_timeout_ms = (pause_timeout_in == 0) ? PAUSE_DECISION_TIMEOUT_MS : pause_timeout_in;
    contract_duration_ms = 0;
    dispensed_ms = 0;
    segment_start_ms = 0;
...
void dispense_on_parar_pressed()
{
    ...
    enter_paused_from_dispensing(millis());
}
...
DispenseEvent dispense_tick()
{
    ...
    if (phase == DISPENSING) {
        if (poll_termo_transition(now, false)) {
            enter_wait_termo_resume(now, true);
            return DISPENSE_EVENT_NONE;
        }
    ...
    if (phase == WAIT_TERMO_RESUME) {
        ...
        if (poll_termo_transition(now, true)) {
            enter_paused_from_wait_termo_resume(now);
        }
    ...
    if (phase == PAUSED) {
        ...
        if (poll_termo_transition(now, false)) {
            enter_wait_termo_resume(now, false);
            return DISPENSE_EVENT_NONE;
        }
