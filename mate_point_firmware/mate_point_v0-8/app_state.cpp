...
void app_state_on_continuar_pressed()
{
    if (state != APP_DISPENSE || drip_tray_is_full() || nobana_tank_empty()) {
        return;
    }
    if (dispense_on_continuar_pressed()) {
        mate_network_publish_status();
    }
}

void app_state_on_finalizar_pressed()
{
    if (state != APP_DISPENSE) {
        return;
    }
    dispense_on_finalizar_pressed();
}
                                   ...
                                   uint32_t pause_timeout_ms)
{
    ...
    if (!dispense_on_command(order_id, duration_ms, pause_timeout_ms)) {
        enter_comprar();
        return false;
    }
