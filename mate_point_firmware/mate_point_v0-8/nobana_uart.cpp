...
static uint32_t s_cooldown_ms = DISPENSE_T_COOLDOWN_MS;
...
static void dispense_tick(uint32_t now)
{
    ...
    case DISPENSE_COOLDOWN_22:
        ...
        if (phase_elapsed(now) >= s_cooldown_ms) {
            dispense_finish();
        }
        break;
...
static bool dispense_abort_internal(uint32_t cooldown_ms)
{
    if (s_dispense == DISPENSE_OFF) {
        return false;
    }
...
{
    return dispense_abort_internal(DISPENSE_T_COOLDOWN_MS);
}

bool nobana_dispense_abort_pause()
{
    return dispense_abort_internal(PAUSE_COOLDOWN_MS);
}
