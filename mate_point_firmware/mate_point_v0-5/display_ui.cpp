...
static bool cargar_dispensing = false;
...
static lv_obj_t *scr_connectivity;
...
static void build_connectivity()
{
    scr_connectivity = lv_obj_create(nullptr);
    lv_obj_set_size(scr_connectivity, UI_SCREEN_W, UI_SCREEN_H);
    lv_obj_clear_flag(scr_connectivity, LV_OBJ_FLAG_SCROLLABLE);

    add_left_image(scr_connectivity, &img_split_image_error_wifi_mqtt);
    lv_obj_t *right = make_panel(scr_connectivity, UI_PANEL_W, UI_PANEL_W, UI_COLOR_GREEN_DARKEST);
...
{
    const bool ok = wifi_ok && mqtt_ok;
    if (!ok) {
        if (!connectivity_blocked) {
            connectivity_blocked = true;
            lv_scr_load(scr_connectivity);
        }
        return;
    }
...
static void set_cargar_mode(bool dispensing)
{
    cargar_dispensing = dispensing;
    if (!cargar_btn_action || !cargar_lbl_estado) {
        return;
    }

    lv_label_set_text(cargar_lbl_estado,
                      dispensing ? UI_STR_ESTADO_CARGANDO : UI_STR_ESTADO_ESPERA);
    lv_label_set_text(lv_obj_get_child(cargar_btn_action, 0),
                      dispensing ? UI_STR_PARAR : UI_STR_INICIAR);
