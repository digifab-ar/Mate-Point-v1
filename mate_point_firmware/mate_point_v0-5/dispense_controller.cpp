...
static float ui_liters_from_elapsed_ms(uint32_t elapsed_ms)
{
    const float sec = elapsed_ms / 1000.0f;
...
static void update_dispense_ui(uint32_t now)
{
    ...
    if (contract_start_ms != 0) {
        const uint32_t elapsed = now - contract_start_ms;
        display_ui_set_dispense_liters(ui_liters_from_elapsed_ms(elapsed));
    }
}
...
{
    if (phase != READY_START || drip_tray_is_full()) {
        return false;
    }
