#include "order_client.h"

#include "config.h"

#include <Arduino.h>
#include <string.h>
#include <ArduinoJson.h>
#include <HTTPClient.h>
#include <WiFi.h>
#include <WiFiClientSecure.h>

static bool post_json(const char *path, const char *body, int *http_code_out, String *response_out)
{
    WiFiClientSecure client;
    client.setInsecure();

    HTTPClient http;
    const String url = String("https://") + SERVER_HOST + path;
    if (!http.begin(client, url)) {
        return false;
    }

    http.addHeader("Content-Type", "application/json");
    http.setTimeout(15000);

    const int code = http.POST(body ? body : "{}");
    const String response = http.getString();
    http.end();

    if (http_code_out) {
        *http_code_out = code;
    }
    if (response_out) {
        *response_out = response;
    }

    return code > 0;
}

static void copy_or_placeholder(char *dst, size_t dst_len, const char *value,
                                const char *placeholder)
{
    if (!dst || dst_len == 0) {
        return;
    }
    const char *src = (value && value[0]) ? value : placeholder;
    strncpy(dst, src, dst_len - 1);
    dst[dst_len - 1] = '\0';
}

bool order_create(char *order_id, size_t order_id_len,
                  char *external_ref, size_t external_ref_len,
                  char *product_desc, size_t product_desc_len,
                  char *price_display, size_t price_display_len)
{
    if (order_id && order_id_len) {
        order_id[0] = '\0';
    }
    if (external_ref && external_ref_len) {
        external_ref[0] = '\0';
    }
    copy_or_placeholder(product_desc, product_desc_len, nullptr, UI_PRODUCT_DESC_PLACEHOLDER);
    copy_or_placeholder(price_display, price_display_len, nullptr, UI_PRODUCT_PRICE_PLACEHOLDER);

    StaticJsonDocument<128> req;
    req["device_id"] = DEVICE_ID;
    char body[128];
    serializeJson(req, body, sizeof(body));

    int code = 0;
    String response;
    if (!post_json("/orders/create", body, &code, &response)) {
        return false;
    }

    if (code != 201) {
        return false;
    }

    StaticJsonDocument<768> doc;
    if (deserializeJson(doc, response)) {
        return false;
    }

    const char *id = doc["order_id"] | "";
    const char *ext = doc["external_reference"] | "";
    if (order_id && order_id_len) {
        strncpy(order_id, id, order_id_len - 1);
        order_id[order_id_len - 1] = '\0';
    }
    if (external_ref && external_ref_len) {
        strncpy(external_ref, ext, external_ref_len - 1);
        external_ref[external_ref_len - 1] = '\0';
    }

    copy_or_placeholder(product_desc, product_desc_len,
                        doc["product_description"] | "", UI_PRODUCT_DESC_PLACEHOLDER);
    copy_or_placeholder(price_display, price_display_len,
                        doc["price_display"] | "", UI_PRODUCT_PRICE_PLACEHOLDER);

    return id[0] != '\0';
}

bool order_cancel(const char *order_id)
{
    if (!order_id || !order_id[0]) {
        return false;
    }

    StaticJsonDocument<128> req;
    req["order_id"] = order_id;
    char body[128];
    serializeJson(req, body, sizeof(body));

    int code = 0;
    String response;
    if (!post_json("/orders/cancel", body, &code, &response)) {
        return false;
    }

    return code == 200;
}
